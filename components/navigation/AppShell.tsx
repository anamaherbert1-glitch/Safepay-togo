...
import { getNotificationSoundEnabled, getTheme, type CyenooTheme } from "@/lib/preferences";
import { playNotificationSound } from "@/lib/notificationSound";
import { useCyenooLanguage } from "@/lib/i18n";
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const cachedComplete =
          ...
          (localStorage.getItem(PROFILE_FLAG) === "1" || sessionStorage.getItem("cyenoo-profile-complete") === "1");

        if (!cachedComplete) {
          const { data: rawProfile, error: profileError } = await withTimeout(supabase.rpc("get_my_profile"));
          if (!active) return;
          if (profileError) {
            console.warn("[Cyenoo] get_my_profile", profileError.message);
            setAuthorized(true);
            return;
          }
          ...
          try {
            ...
            sessionStorage.setItem("cyenoo-profile-complete", "1");
          } catch {}
        ...
        try {
          ...
          if (
            ...
            sessionStorage.getItem("cyenoo-biometric-unlocked") !== "1"
          ) {
          ...
          console.warn("[Cyenoo] biometric gate skipped", bioErr);
        }

        setAuthorized(true);
        notificationChannel = supabase
          .channel(`cyenoo-global-notifications-${user.id}`)
          .on(
            "postgres_changes",
    ...
    const onThemeUpdated = (event: Event) => {
      const custom = event as CustomEvent<{ theme?: CyenooTheme }>;
    ...
    window.addEventListener("cyenoo-avatar-updated", onAvatarUpdated);
    window.addEventListener("cyenoo-theme-updated", onThemeUpdated);
    window.addEventListener("pagehide", clearBiometricOnPageHide);
    return () => {
      active = false;
      listener.subscription.unsubscribe();
      if (notificationChannel) void supabase.removeChannel(notificationChannel);
      window.removeEventListener("cyenoo-avatar-updated", onAvatarUpdated);
      window.removeEventListener("cyenoo-theme-updated", onThemeUpdated);
      window.removeEventListener("pagehide", clearBiometricOnPageHide);
    };
  }, [router, pathname]);
  ...
  if (checking || !authorized) {
    return (
      <div className="cyenoo-shell cyenoo-dashboard">
        <div className="sp-page-loading" role="status" aria-label={language === "en" ? "Loading" : "Chargement"}>
    ...
    <div className="cyenoo-shell cyenoo-dashboard">
      {pathname !== "/dashboard" && (
        <header className="sp-header">
          <button className="sp-back" onClick={goBack} aria-label={language === "en" ? "Back" : "Retour"}>
            <BackIcon />
          </button>
      ...
      <nav className="sp-bottom-nav" aria-label={language === "en" ? "Main navigation" : "Navigation principale"}>
        {items.map((item) => {
          ...
          return (
            ...
            >
              <span className={isProfile ? "sp-nav-profile-icon" : ""} aria-hidden="true">
                {isProfile && avatarUrl ? (
                  <img src={avatarUrl} alt="" onError={() => setAvatarUrl("")} />
                ) : (
                  item.icon
                )}
              </span>
