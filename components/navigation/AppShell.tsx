...
import { getNotificationSoundEnabled, getTheme, type SafePayTheme } from "@/lib/preferences";
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => { let active = true; let notificationChannel: any = null; const supabase = createClient(); const load = async () => { setChecking(true); try { const claimsResult = await withTimeout(supabase.auth.getClaims()); if (!active) return; if (claimsResult.error || !claimsResult.data?.claims) { router.replace("/login"); return; } const { data: { session } } = await supabase.auth.getSession(); const user = session?.user; if (!user) { router.replace("/login"); return; } if (!user.phone_confirmed_at) { router.replace("/auth?resume=1"); return; } const cachedComplete = typeof window !== "undefined" && sessionStorage.getItem("safepay-profile-complete") === "1"; if (!cachedComplete) { const { data: rawProfile, error: profileError } = await withTimeout(supabase.rpc("get_my_profile")); if (!active) return; if (profileError) throw profileError; const profile = Array.isArray(rawProfile) ? rawProfile[0] : rawProfile; if (!profile || !profile.full_name || !profile.phone_verified) { router.replace("/auth?resume=1"); return; } try { sessionStorage.setItem("safepay-profile-complete", "1"); } catch {} setAvatarUrl(profile.avatar_url || ""); }
      const { data: security } = await withTimeout(supabase.from("user_security").select("biometric_enabled").eq("user_id", user.id).maybeSingle());
      if (security?.biometric_enabled && pathname !== "/profile" && typeof window !== "undefined" && sessionStorage.getItem("safepay-biometric-unlocked") !== "1") {
        const auth = supabase.auth as any;
        if (typeof auth.signInWithPasskey !== "function") throw new Error("Biométrie indisponible.");
        const { data: passkeys, error: passkeyListError } = typeof auth.passkey?.list === "function" ? await auth.passkey.list() : { data: null, error: null };
        if (passkeyListError) throw passkeyListError;
        if (!Array.isArray(passkeys) || passkeys.length === 0) { router.replace("/profile?biometric=setup"); return; }
        const { error: passkeyError } = await auth.signInWithPasskey();
        if (passkeyError) throw passkeyError;
        try { sessionStorage.setItem("safepay-biometric-unlocked", "1"); } catch {}
      }
      setAuthorized(true); notificationChannel = supabase.channel(`safepay-global-notifications-${user.id}`).on("postgres_changes", { event: "INSERT", schema: "public", table: "notifications", filter: `user_id=eq.${user.id}` }, async () => { if (active && getNotificationSoundEnabled()) await playNotificationSound(); }).subscribe(); } catch { if (active) router.replace("/login"); } finally { if (active) setChecking(false); } }; load(); const { data: listener } = supabase.auth.onAuthStateChange((_event, session) => { if (!session) { setAuthorized(false); try { sessionStorage.removeItem("safepay-profile-complete"); sessionStorage.removeItem("safepay-biometric-unlocked"); } catch {} router.replace("/login"); } }); const onAvatarUpdated = (event: Event) => { const custom = event as CustomEvent<{ url?: string }>; setAvatarUrl(custom.detail?.url || ""); }; const onThemeUpdated = (event: Event) => { const custom = event as CustomEvent<{ theme?: SafePayTheme }>; const theme = custom.detail?.theme || getTheme(); if (typeof document !== "undefined") { if (theme === "system") delete document.documentElement.dataset.theme; else document.documentElement.dataset.theme = theme; } }; const clearBiometricOnPageHide = () => { try { sessionStorage.removeItem("safepay-biometric-unlocked"); } catch {} }; window.addEventListener("safepay-avatar-updated", onAvatarUpdated); window.addEventListener("safepay-theme-updated", onThemeUpdated); window.addEventListener("pagehide", clearBiometricOnPageHide); return () => { active = false; listener.subscription.unsubscribe(); if (notificationChannel) void supabase.removeChannel(notificationChannel); window.removeEventListener("safepay-avatar-updated", onAvatarUpdated); window.removeEventListener("safepay-theme-updated", onThemeUpdated); window.removeEventListener("pagehide", clearBiometricOnPageHide); }; }, [router, pathname]);
