...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setChecking(true);
      try {
        // Restore durable session from localStorage (retry once for cold start)
        ...
        notificationChannel = supabase
          .channel(`safepay-global-notifications-${user.id}`)
          .on(
            "postgres_changes",
    ...
    // Only explicit SIGNED_OUT clears the session — never on transient null
    ...
    const onThemeUpdated = (event: Event) => {
      const custom = event as CustomEvent<{ theme?: SafePayTheme }>;
    ...
    return () => {
      ...
      window.removeEventListener("safepay-theme-updated", onThemeUpdated);
