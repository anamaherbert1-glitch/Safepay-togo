...
export default function ProfilePage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const [profileResult, accountResult, securityResult] = results;
      if (profileResult.status === "fulfilled") {
        if (profileResult.value.error) setError(profileResult.value.error.message);
        else setProfile(profileResult.value.data);
      } else setError("Impossible de charger le profil SafePay.");

      if (accountResult.status === "fulfilled") {
        if (accountResult.value.error && !error) setError(accountResult.value.error.message);
        else setAccount(accountResult.value.data);
      }

      if (securityResult.status === "fulfilled" && !securityResult.value.error) setSecurity(securityResult.value.data);
      setEmail(user.email ?? "");
      setLoading(false);
    };
    load();
  ...
  return <AppShell><section className="sp-page">
    ...
    {loading ? <div className="sp-page-loading"><span className="sp-loader"/>Chargement du profil…</div> : <>
      <section className="sp-section-card sp-profile-card"><label className="sp-avatar-upload" title="Changer la photo de profil"><div className="sp-avatar-frame">{avatarUrl ? <img src={avatarUrl} alt="Photo de profil"/> : <ProfileIcon />}</div><input type="file" accept="image/*" onChange={uploadAvatar} disabled={uploading}/><span>{uploading ? "Envoi…" : "Modifier la photo"}</span></label><h2>{fullName}</h2><p className="sp-muted">{role}</p></section>
