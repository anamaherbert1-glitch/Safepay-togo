  ...
  return (
    <main className="safepay-shell safepay-dashboard">
      <header className="sp-header">
        <div className="sp-brand"><span className="sp-brand-mark">S</span><strong>SafePay</strong></div>
      ...
      <section className="sp-content">
        ...
        <section className="sp-balance-card" aria-label="Solde du portefeuille">
          <div className="sp-balance-glow" />
          <div className="sp-balance-top"><span>Solde total</span><button className="sp-eye" aria-label="Afficher le solde">◉</button></div>
          <div className="sp-total">{wallet ? money(Number(wallet.balance) + Number(wallet.locked_balance), currency) : "—"}</div>
          <div className="sp-balance-meta"><div><span>Solde disponible</span><strong>{wallet ? money(Number(wallet.balance), currency) : "—"}</strong></div><div><span>Fonds bloqués</span><strong>{wallet ? money(Number(wallet.locked_balance), currency) : "—"}</strong></div></div>
        </section>
        <section className="sp-quick-actions" aria-label="Actions rapides">
          <button className="sp-action" onClick={() => router.push("/wallet")}><span className="sp-action-icon blue"><PlusIcon /></span><span>Recharger</span></button>
          <button className="sp-action" onClick={() => router.push("/transactions/new")}><span className="sp-action-icon green"><ArrowUpIcon /></span><span>Envoyer</span></button>
          <button className="sp-action" onClick={() => router.push("/transactions")}><span className="sp-action-icon violet"><ArrowDownIcon /></span><span>Recevoir</span></button>
          <button className="sp-action" onClick={() => router.push("/services")}><span className="sp-action-icon orange"><MoreIcon /></span><span>Plus</span></button>
        </section>
        ...
        <section className="sp-section-card"><div className="sp-section-head"><h2>Services rapides</h2><button onClick={() => router.push("/services")}>Voir tout</button></div><div className="sp-services">
          <button onClick={() => router.push("/wallet")}><span className="service-icon orange"><WalletIcon /></span><span>Mobile Money</span></button>
          <button onClick={() => router.push("/services")}><span className="service-icon violet"><MoreIcon /></span><span>Paiements</span></button>
          <button onClick={() => router.push("/transactions/new")}><span className="service-icon green"><ArrowUpIcon /></span><span>Transfert</span></button>
      ...
      <nav className="sp-bottom-nav" aria-label="Navigation principale">
        <button className="active"><span>⌂</span><small>Accueil</small></button>
        <button onClick={() => router.push("/transactions")}><span>↕</span><small>Transactions</small></button>
        <button onClick={() => router.push("/notifications")}><span>♧</span><small>Notifications</small></button>
        <button onClick={() => router.push("/profile")}><span><ProfileIcon /></span><small>Profil</small></button>
      </nav>
    </main>
  );
}
