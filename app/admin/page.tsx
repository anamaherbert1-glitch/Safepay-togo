...
export default function AdminPage(){
 const router=useRouter();const supabase=useMemo(()=>createClient(),[]);const [settings,setSettings]=useState<Setting[]>(DEFAULT_SETTINGS);const [flags,setFlags]=useState<Record<string,boolean>>({});const [reason,setReason]=useState("");const [loading,setLoading]=useState(true);const [saving,setSaving]=useState<string|null>(null);const [message,setMessage]=useState("");const [error,setError]=useState("");const [summary,setSummary]=useState<Record<string,unknown>|null>(null);
 useEffect(()=>{(async()=>{const {data:{user}}=await supabase.auth.getUser();if(!user){router.replace("/login?next=/admin");return;}const [{data:s,error:se},{data:sum,error:sume},{data:f,error:fe}]=await Promise.all([supabase.rpc("get_public_settings"),supabase.rpc("get_admin_dashboard_summary"),supabase.from("feature_flags").select("key,enabled")]);if(se)setError(se.message);else if(s)setSettings(DEFAULT_SETTINGS.map(i=>({...i,value:s[i.key]??i.value})));if(sume&&sume.message!=="admin_required")setError(sume.message);else setSummary((sum??null) as Record<string,unknown>|null);if(fe)setError(fe.message);else setFlags(Object.fromEntries((f??[]).map(r=>[r.key,r.enabled])));setLoading(false)})()},[router,supabase]);
 async function save(item:Setting){setSaving(item.key);setMessage("");setError("");const {error:e}=await supabase.rpc("admin_set_app_setting",{p_key:item.key,p_value_numeric:item.type==="number"?Number(item.value):null,p_value_text:null,p_value_boolean:item.type==="boolean"?Boolean(item.value):null,p_reason:reason.trim()||"Modification depuis le Dashboard Admin"});if(e)setError(e.message);else{setSettings(all=>all.map(x=>x.key===item.key?item:x));setMessage(`${item.label} mis à jour.`)}setSaving(null)}
