...
function Icon({ name }: { name: string }) {
  const map: Record<string, string> = { refresh: "↻", close: "×" };
...
export default function AdminDashboardLegacy() {
  ...
  const rpc = useCallback(async (name: string, args: Record<string, any> = {}) => {
  ...
  const loadFlags = useCallback(async () => {
    ...
    DEFAULT_FLAGS.forEach((f) => { map[f.key] = false; });
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      if (module === "overview" || module === "revenue") {
        setStats((await rpc("admin_dashboard_stats")) || {});
        setRows([]);
  ...
  async function toggleFlag(key: string, enabled: boolean) {
    setActionBusy(true);
    try {
      await rpc("admin_set_feature_flag", { p_key: key, p_enabled: enabled, p_reason: "Dashboard Admin Cyenoo" });
      setFlags((p) => ({ ...p, [key]: enabled }));
  ...
  async function runDialogAction() {
    ...
    try {
      if (dialog.kind === "withdrawal") {
        await rpc("admin_update_withdrawal_status", { p_withdrawal_id: dialog.row.id, p_status: choice, p_reason: text || "Admin Cyenoo" });
        setToast("Retrait mis à jour.");
      } else if (dialog.kind === "notify") {
        if (!text.trim()) throw new Error("Message requis");
        await rpc("admin_send_notification", { p_user_id: dialog.row.id, p_title: "Message Cyenoo", p_message: text.trim() });
        setToast("Notification envoyée.");
      } else if (dialog.kind === "user-status") {
        const next = !(dialog.row.is_active ?? true);
        await rpc("admin_set_account_status", { p_user_id: dialog.row.id, p_is_active: next, p_reason: text || (next ? "Réactivation" : "Suspension") });
        setToast(next ? "Compte réactivé." : "Compte suspendu.");
      } else if (dialog.kind === "dispute") {
        if (!text.trim()) throw new Error("Justification requise");
        await rpc("admin_resolve_dispute", { p_dispute_id: dialog.row.id, p_resolution: text.trim(), p_resolution_action: choice });
        setToast("Litige résolu.");
      }
      setDialog(null);
      setText("");
      await load();
  ...
  if (!ready) return <main className="adm-app"><AdminStyles /><div className="adm-loading">Chargement Cyenoo Admin…</div></main>;
  if (!allowed) {
    return (
      <main className="adm-app">
        <AdminStyles />
        <div className="adm-loading" style={{ flexDirection: "column", gap: 12 }}>
          <div className="adm-logo" style={{ display: "grid", placeItems: "center" }}><CyenooMark size={28} /></div>
  ...
  const showTable = !["overview", "finance", "revenue", "settings", "features", "security"].includes(module);
  const actionModules = ["withdrawals", "users", "disputes"].includes(module);
    ...
    <main className="adm-app">
      ...
      <aside className="adm-sidebar">
        <div className="adm-brand">
          <div className="adm-brand-mark" style={{ padding: 6 }}><CyenooMark size={30} /></div>
          <div><strong>Cyenoo</strong><span>ADMIN CONSOLE</span></div>
        </div>
        <nav>
          {modules.map(([id, label]) => (
            <button key={id} className={module === id ? "active" : ""} onClick={() => setModule(id)}>{label}</button>
          ))}
        </nav>
        <div className="adm-side-footer">
          <button className="adm-secondary" onClick={() => router.push("/dashboard")}>Retour à Cyenoo</button>
        </div>
      </aside>
      ...
      <section className="adm-main">
        <header className="adm-header">
          ...
          <div className="adm-header-right">
            <div className="adm-live"><i /> Backend Cyenoo connecté</div>
            <button className="adm-refresh" onClick={() => void load()} disabled={busy}><Icon name="refresh" /> Actualiser</button>
          </div>
        </header>
        ...
        {module === "revenue" && (
          <div className="adm-hero">
            <div className="adm-heading"><h2>Revenus Cyenoo</h2><p>RPC administratives Cyenoo.</p></div>
            <div className="adm-kpis">
              {[["Volume total", stats.volume_total ?? stats.total_volume, true], ["Frais collectés", stats.fees_total ?? stats.total_fees, true], ["Commissions", stats.commissions_total, true]].map(([label, value, isMoney]) => (
                <div className="adm-kpi" key={String(label)}><div className="adm-kpi-top"><span>{label}</span></div><strong>{isMoney ? money(value) : value ?? "—"}</strong></div>
              ))}
            </div>
          </div>
        )}

        {module === "finance" && <div className="adm-panel" style={{ padding: 8 }}><FinanceCommission /></div>}
        {module === "settings" && <div className="adm-panel" style={{ padding: 8 }}><AdminSettings /></div>}

        {module === "features" && (
          <div className="adm-hero">
            <div className="adm-heading"><span className="adm-eyebrow">FEATURE FLAGS</span><h2>Contrôle des fonctionnalités</h2></div>
            <div style={{ display: "grid", gap: 12, marginTop: 16 }}>
              {DEFAULT_FLAGS.map((f) => {
                const on = !!flags[f.key];
                return (
                  <div key={f.key} style={{ display: "flex", justifyContent: "space-between", gap: 16, padding: "14px 16px", borderRadius: 14, border: "1px solid rgba(135,185,235,.15)", background: "rgba(13,29,48,.8)" }}>
                    <div><strong>{f.label}</strong><div style={{ color: "#8398ad", fontSize: 12 }}>{f.description}</div></div>
                    <button className={on ? "adm-primary" : "adm-secondary"} disabled={actionBusy} onClick={() => void toggleFlag(f.key, !on)}>{on ? "ON" : "OFF"}</button>
                  </div>
                );
              })}
            </div>
        ...
        {module === "security" && (
          <div className="adm-hero"><div className="adm-heading"><h2>Sécurité</h2><p>Garde-fous Cyenoo via RPC admin.</p></div></div>
        )}

        {module === "users" && (
          <div className="adm-data-toolbar">
            <input className="adm-search" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Rechercher un utilisateur…" />
            <button className="adm-primary" onClick={() => void load()} disabled={busy}>Chercher</button>
          </div>
        )}

        {showTable && (
          <div className="adm-table-wrap">
            {busy ? <div className="adm-loading">Chargement…</div> : rows.length === 0 ? <div className="adm-empty">Aucune donnée.</div> : (
              <table>
                <thead><tr>{columns.map((c) => <th key={c}>{c.replace(/_/g, " ")}</th>)}{actionModules && <th>Actions</th>}</tr></thead>
                <tbody>
                  {rows.map((r, i) => (
                    <tr key={r.id || r.user_id || i}>
                      {columns.map((c) => (
                        <td key={c}>{/amount|balance|volume|fee/i.test(c) ? money(r[c]) : /created_at|updated_at/i.test(c) ? date(r[c]) : r[c] == null || r[c] === "" ? "—" : typeof r[c] === "object" ? JSON.stringify(r[c]) : String(r[c])}</td>
                      ))}
                      {module === "withdrawals" && <td><button className="adm-secondary" onClick={() => { setChoice("completed"); setText(""); setDialog({ kind: "withdrawal", row: r }); }}>Traiter</button></td>}
                      {module === "disputes" && <td><button className="adm-secondary" onClick={() => { setChoice("refund_buyer"); setText(""); setDialog({ kind: "dispute", row: r }); }}>Résoudre</button></td>}
                      {module === "users" && (
                        <td style={{ display: "flex", gap: 6 }}>
                          <button className="adm-secondary" onClick={() => { setText(""); setDialog({ kind: "notify", row: r }); }}>Notifier</button>
                          <button className="adm-secondary" onClick={() => { setText(""); setDialog({ kind: "user-status", row: r }); }}>{(r.is_active ?? true) ? "Suspendre" : "Activer"}</button>
                        </td>
                      )}
                    </tr>
                  ))}
          ...
          <div className="adm-modal">
            <div className="adm-modal-head">
              <div>
                ...
                <h3>{dialog.kind === "withdrawal" ? "Traitement du retrait" : dialog.kind === "notify" ? "Notifier" : dialog.kind === "dispute" ? "Résolution du litige" : (dialog.row.is_active ?? true) ? "Suspendre" : "Réactiver"}</h3>
              </div>
            ...
            <div className="adm-modal-body">
              ...
              {dialog.kind === "withdrawal" && (
                <label className="adm-modal-label">Statut<select value={choice} onChange={(e) => setChoice(e.target.value)}><option value="processing">Processing</option><option value="completed">Completed</option><option value="failed">Failed</option><option value="cancelled">Cancelled</option></select></label>
              )}
              {dialog.kind === "dispute" && (
                <label className="adm-modal-label">Décision<select value={choice} onChange={(e) => setChoice(e.target.value)}><option value="refund_buyer">Rembourser acheteur</option><option value="release_seller">Libérer vendeur</option></select></label>
              )}
              <label className="adm-modal-label">{dialog.kind === "notify" ? "Message" : "Justification"}<textarea value={text} onChange={(e) => setText(e.target.value)} /></label>
            </div>
            <div className="adm-modal-foot">
              <button className="ghost" onClick={() => setDialog(null)}>Annuler</button>
              <button className="adm-primary" disabled={actionBusy || ((dialog.kind === "notify" || dialog.kind === "dispute") && !text.trim())} onClick={() => void runDialogAction()}>{actionBusy ? "…" : "Confirmer"}</button>
            </div>
          </div>
        </div>
      )}
