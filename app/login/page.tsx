...
export default function LoginPage() {
  ...
  async function sendPhoneOtp(e: FormEvent) {
    ...
    try {
      ...
      setPhoneE164(result.e164); setOtp(""); setOtpSent(true); setError("Code OTP envoyé. Entrez-le pour accéder à votre compte.");
    } catch (err) { setError(friendlyAuthError(err instanceof Error ? err.message : "Impossible d’envoyer le code OTP.")); }
  ...
  async function verifyPhoneOtp(e: FormEvent) {
    ...
    } catch (err) { setError(friendlyAuthError(err instanceof Error ? err.message : "Impossible de vérifier le code.")); }
    finally { setBusy(false); }
  }

  return <main className="safepay-shell auth-screen"><header className="auth-header"><button className="safepay-icon" onClick={() => router.replace("/")} aria-label="Retour">←</button><strong>SafePay</strong></header><section className="auth-content"><div className="safepay-card auth-card"><div className="auth-kicker">SafePay V5</div><h1>Se connecter</h1><p className="sp-muted">La connexion SafePay se fait par numéro de téléphone + OTP. Aucun mot de passe email n’est demandé ici.</p>
    <form onSubmit={otpSent ? verifyPhoneOtp : sendPhoneOtp} className="sp-form"><label>Numéro de téléphone<div className="phone-row"><select value={countryCode} onChange={e => { setCountryCode(e.target.value); setOtpSent(false); setOtp(""); setError(""); }} aria-label="Pays">{SAFE_PAY_COUNTRIES.map(c => <option key={c.code} value={c.code}>{c.flag} {c.callingCode}</option>)}</select><input value={phoneLocal} onChange={e => setPhoneLocal(onlyPhoneCharacters(e.target.value))} placeholder="90 XX XX XX" inputMode="tel" autoComplete="tel" required /></div></label><div className="phone-country-meta">{country.flag} {country.callingCode} · {country.name} · {country.currency}</div>{otpSent && <label>Code OTP<input value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g, ""))} maxLength={6} inputMode="numeric" autoComplete="one-time-code" placeholder="000000" required /></label>}<button className="safepay-primary" disabled={busy}>{busy ? (otpSent ? "Vérification…" : "Envoi…") : (otpSent ? "Vérifier le code" : "Envoyer le code")}</button></form>
    {error && <p className="sp-form-error" role="alert">{error}</p>}<p className="auth-footer">Pas encore de compte ? <button type="button" onClick={() => router.push("/auth")}>Créer un compte</button></p>
  </div></section></main>;
}
