...
function DisputeIcon(){return <svg viewBox="0 0 24 24" width="23" height="23" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M12 3 20 7v5c0 4.5-3 7.7-8 9-5-1.3-8-4.5-8-9V7l8-4Z"/><path d="M12 8v4M12 16h.01"/></svg>}
function InvoiceIcon(){return <svg viewBox="0 0 24 24" width="23" height="23" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M6 3h12v18l-3-2-3 2-3-2-3 2V3Z"/><path d="M9 8h6M9 12h6"/></svg>}
function SupportIcon(){return <svg viewBox="0 0 24 24" width="23" height="23" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round"><path d="M4 5h16v11H8l-4 4V5Z"/><path d="M8 9h8M8 12h5"/></svg>}
const services=[
 ...
 ["Litiges","Suivre vos dossiers de protection","violet","/disputes",<DisputeIcon/>],
 ["Factures","Consulter vos documents financiers","blue","/invoices",<InvoiceIcon/>],
 ["Support","Contacter l'équipe SafePay","green","/support",<SupportIcon/>],
 ["Paramètres","Thème, langue, devise et sécurité","blue","/settings",<SettingsIcon/>],
] as const;
export default function ServicesPage(){const router=useRouter();return <main className="safepay-shell safepay-dashboard"><header className="sp-header"><button className="sp-back" onClick={()=>router.back()} aria-label="Retour"><span className="sp-back-icon" aria-hidden="true"/></button><strong>Services</strong><span style={{width:40}}/></header><section className="sp-content"><p className="sp-eyebrow">Services</p><h1 className="sp-title">Tous vos services</h1><section className="sp-services-page">{services.map(([title,description,color,href,icon])=><button key={title} className="sp-service-card" onClick={()=>router.push(href)}><span className={`service-icon ${color}`}>{icon}</span><span><strong>{title}</strong><small>{description}</small></span><b>›</b></button>)}</section></section></main>}
