...
export default function AuthPage() {
  ...
  async function submitPhone(event: FormEvent) {
    ...
    if (!session) { setBusy(false); return setMessage("Session expirée. Reconnectez-vous pour continuer."); }
    const { data, error } = await supabase.functions.invoke("phone-otp", { body: { action: "send", phone: result.e164 } });
    setBusy(false);
    if (error || data?.error) return setMessage(data?.error || error?.message || "Impossible d’envoyer le code.");
    setPhoneE164(result.e164); setOtp(""); setStep("otp");
    setMessage("Code envoyé par SMS. Il expire dans 5 minutes.");
  }

  async function submitOtp(event: FormEvent) {
    event.preventDefault(); setMessage("");
    if (!/^\d{6}$/.test(otp)) return setMessage("Entrez le code à 6 chiffres reçu par SMS.");
    setBusy(true);
    const supabase = createClient();
    const { data, error } = await supabase.functions.invoke("phone-otp", { body: { action: "verify", phone: phoneE164, code: otp } });
    setBusy(false);
    if (error || data?.error) return setMessage(data?.error || error?.message || "Code incorrect.");
    setStep("profile"); setMessage("Téléphone vérifié. Vous pouvez maintenant créer votre profil SafePay.");
  }

  function submitProfile(event: FormEvent) {
    event.preventDefault(); setMessage("");
    if (!/^[\p{L}][\p{L}\s'’-]{1,79}$/u.test(fullName.trim())) return setMessage("Le nom complet doit contenir uniquement des caractères alphabétiques et des espaces.");
    setMessage(`Profil ${role === "seller" ? "Vendeur" : "Client"} prêt. La prochaine étape est le branchement du bootstrap V5 profile → account → wallet.`);
  }
  ...
  return <main className="safepay-shell" style={{ minHeight: "100vh", padding: 20 }}>
    <header style={{ display: "flex", alignItems: "center", gap: 12, height: 56 }}>
      {step !== "method" && <button className="safepay-icon" onClick={back} aria-label="Retour">←</button>}
      <strong style={{ fontSize: 21 }}>SafePay</strong>
    </header>
    <section style={{ paddingTop: 28 }}><div className="safepay-card" style={{ padding: 22 }}>
      <div style={{ color: "var(--sp-muted)", fontSize: 13 }}>SafePay V5</div><h1 style={{ margin: "8px 0" }}>{title}</h1>
