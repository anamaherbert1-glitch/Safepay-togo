...
export default function SupportPage() {
  ...
  async function createTicket() {
    ...
    if (ticketError || !ticket) {
      setError(ticketError?.message ?? "Impossible de créer le ticket.");
      setBusy(false);
      return;
    }
    await supabase
      .from("support_messages")
      .insert({ ticket_id: ticket.id, sender_id: user.id, sender_type: "user", message: text });
    setNewTicket("");
    await loadTickets();
  ...
  return (
    <AppShell>
      <section className="sp-page">
        <p className="sp-eyebrow">Assistance Cyenoo</p>
        ...
        <section className="sp-section-card">
          ...
          <div className="sp-form" style={{ marginTop: 10 }}>
            <textarea
              rows={4}
              value={newTicket}
            ...
            />
            <button className="safepay-primary sp-submit" disabled={busy} onClick={createTicket}>
              {busy ? "Envoi…" : "Contacter Cyenoo"}
            </button>
          </div>
        </section>
        ...
        {loading ? (
          <section className="sp-section-card">
            <p className="sp-muted">Chargement…</p>
          </section>
        ) : tickets.length === 0 ? (
          <section className="sp-section-card sp-empty-list">
            <strong>Aucun ticket</strong>
            <span>Vos échanges avec l'équipe Cyenoo apparaîtront ici.</span>
          </section>
        ) : (
          <section className="sp-section-card">
            <div className="sp-section-head">
            ...
            <div className="sp-notification-list">
              {tickets.map((ticket) => (
                <button
                  key={ticket.id}
                  className={`sp-notification-row${selected?.id === ticket.id ? " read" : ""}`}
        ...
        {selected && (
          <section className="sp-section-card">
            <div className="sp-section-head">
              <h2>Conversation</h2>
              <button className="sp-secondary-button" onClick={() => setSelected(null)}>
                Fermer
              </button>
            </div>
            <div className="sp-notification-list" style={{ marginTop: 12 }}>
              {messages.map((message) => (
                <div key={message.id} className="sp-notification-row" style={{ opacity: 1 }}>
                  <span className="sp-notification-dot" />
                  <span>
                    <strong>{message.sender_type === "admin" ? "Cyenoo" : "Vous"}</strong>
            ...
            <div className="sp-form" style={{ marginTop: 12 }}>
              ...
              />
              <button
                className="safepay-primary sp-submit"
