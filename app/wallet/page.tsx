...
export default function WalletPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (walletError || !data) { if (active) { setError(walletError?.message || "Wallet introuvable."); setLoading(false); } return; }
  ...
  return <main className="safepay-shell safepay-dashboard">
    ...
    <section className="sp-content">
      <section className="sp-balance-card"><div className="sp-balance-top"><span>Solde disponible</span></div><div className="sp-total">{wallet ? money(Number(wallet.balance), currency) : "—"}</div><div className="sp-balance-meta"><div><span>Fonds bloqués</span><strong>{wallet ? money(Number(wallet.locked_balance), currency) : "—"}</strong></div><div><span>Retraits réservés</span><strong>{wallet ? money(Number(wallet.withdrawal_reserved || 0), currency) : "—"}</strong></div></div></section>
      <section className="sp-quick-actions" aria-label="Actions du Wallet"><button className="sp-action" onClick={() => router.push("/wallet/deposit")}><span className="sp-action-icon blue">＋</span><span>Recharger</span></button><button className="sp-action" onClick={() => router.push("/wallet/withdraw")}><span className="sp-action-icon orange">↓</span><span>Retirer</span></button></section>
      <section className="sp-section-card"><div className="sp-section-head"><h2>Historique du portefeuille</h2></div>{loading ? <p className="sp-muted">Chargement…</p> : ledger.length === 0 ? <p className="sp-muted">Aucune opération financière dans le ledger.</p> : <div className="sp-transaction-list">{ledger.map(entry => { const positive = ["credit","refund","deposit"].includes(entry.entry_type); return <div className="sp-transaction-row" key={entry.id}><span className="sp-tx-icon">{positive ? "+" : "−"}</span><span className="sp-tx-main"><strong>{entryLabels[entry.entry_type] ?? entry.entry_type}</strong><small>{entry.description ?? "Opération SafePay"}</small><small>{new Date(entry.created_at).toLocaleString("fr-FR")}</small></span><span className="sp-tx-side"><strong>{positive ? "+" : "−"}{money(Number(entry.amount), currency)}</strong><small>Solde : {money(Number(entry.balance_after), currency)}</small></span></div>; })}</div>}</section>
