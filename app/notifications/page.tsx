...
export default function NotificationsPage() {
  ...
  async function markRead(id: string) {
    const s = createClient();
    const { error: e } = await s.rpc("mark_notification_read", { p_notification_id: id });
    if (e) {
      setError(e.message);
      return;
    }
  ...
  return (
    <AppShell>
      <section className="sp-page">
        <p className="sp-eyebrow">Centre d'alertes</p>
        <h1 className="sp-title">Notifications</h1>

        {loading ? (
          <div className="sp-page-loading" role="status" aria-label="Chargement">
          ...
          <section className="sp-notification-list">
            {items.map((item) => (
              <button
                key={item.id}
                className={`sp-notification-row${item.is_read ? " read" : ""}`}
